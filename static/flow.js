...
d3.csv("static/finance.csv").then(function (data) {
  ...
  if (income >= monthly[monthly.length - 1].value) {
    ...
    graph.links.push({
      "source": "income",
      "target": "monthly",
      "value": +monthly[monthly.length - 1].value
    })
    ...
    if (inleft >= daily[daily.length - 1].value) {
      graph.links.push({
        "source": "income",
        "target": "daily",
        "value": +daily[daily.length - 1].value
      })
  ...
  toremove.forEach(el => {
    graph.nodes = graph["nodes"].filter(function (value, index, arr) {
